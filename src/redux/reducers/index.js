const reducer = (state = {}, action = "") => {
  switch (action.type) {
    ...
    case "UPDATE_FEATURES":
      const updated_indexes = action.features
      ...
      return Object.assign({}, state, {
        features: [
          ...state.features.filter(
            feature => !updated_indexes.includes(feature.id)
          ),
          ...action.features
        ]
      });
    case "DELETE_FEATURES":
      const deleted_indexes = action.features
        .map(feature => feature.id)
        .filter(Boolean);
      return Object.assign({}, state, {
        features: [
          ...state.features.filter(
            feature => !deleted_indexes.includes(feature.id)
          )
        ]
      });
    default:
